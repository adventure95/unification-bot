# bot.py
# Final version for "The Learning Circle" — clean, safe, working — 2025-12-05
...
try:
    ...
    raise ValueError("❌ ADMIN_LOG_CHAT_ID must be a valid integer")
...
async def log_new_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if old_status == "left" and new_status == "member":
        ...
        try:
            ...
            logging.warning(f"Could not DM {user.full_name}: {e}")

        welcome_text = (
            f"👋 Welcome, {user.mention_html()}!\n\n"
            "Please verify for <b>The Learning Circle</b>:\n"
            "1️⃣ Tap → @ROTL_Ini420kY\n"
            "2️⃣ Press <b>Start</b>\n"
            "3️⃣ Follow the steps!"
        )
        try:
            await context.bot.send_message(chat_id=chat.id, text=welcome_text, parse_mode=ParseMode.HTML)
        except Exception as e:
        ...
        log_msg = (
            ...
            f"📅 {join_time.strftime('%Y-%m-%d at %H:%M')}"
        )
        try:
            await context.bot.send_message(
                chat_id=ADMIN_LOG_CHAT_ID,
...
async def get_room(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "➡️ What’s your <b>roll number</b>? (e.g., 5)"
    )
    return ROLL
...
async def get_roll(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        ...
        text=(
            ...
            f"🔔 Promote as restricted admin with title <code>Room {room} • #{roll}</code>"
        ),
        parse_mode=ParseMode.HTML
    )
    await update.message.reply_text(
...
async def status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_owner(update.effective_user.id):
        await update.message.reply_text("ℹ️ Owner-only command.", quote=False)
        return
    pending_count = len([u for u in context.bot_data.get("pending", {}).values() if not u.get("verified")])
    await update.message.reply_text(
        f"🔐 <b>Owner Status</b>\n"
        f"🟢 Bot: Active\n"
...
async def list_pending(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    unverified = [f"• {data['name']} (joined {data['join_time'].strftime('%m-%d')})" for uid, data in pending.items() if not data.get("verified")]
    if not unverified:
